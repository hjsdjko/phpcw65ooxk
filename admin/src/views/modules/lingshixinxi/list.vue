<template>
	<div class="main-content" :style='{"color":"#918C8C","padding":"20px 30px 30px","fontSize":"14px"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"0px","margin":"0px","overflow":"hidden","borderRadius":"0px","background":"none","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px","boxShadow":"0 1px 0px rgba(0,0,0,0.12), 0 1px 1px rgba(0,0,0,0.24)","textAlign":"center","flexWrap":"wrap","background":"#fff","flexDirection":"row-reverse","display":"flex","width":"100%","fontSize":"inherit","justifyContent":"center"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">零食名称</label>
						<el-input v-model="searchForm.lingshimingcheng" placeholder="零食名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="零食分类" prop="lingshifenlei">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">零食分类</label>
						<el-select clearable v-model="searchForm.lingshifenlei" placeholder="请选择零食分类" >
							<el-option v-for="(item,index) in lingshifenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"padding":"20px","boxShadow":"0 1px 0px rgba(0,0,0,0.12), 0 1px 1px rgba(0,0,0,0.24)","margin":"20px 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row-reverse","background":"#fff","display":"flex","width":"100%","fontSize":"inherit","justifyContent":"center"}'>
					<el-button class="add" v-if="isAuth('lingshixinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('lingshixinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #bababa","padding":"0","boxShadow":"0 1px 0px rgba(0,0,0,0.12), 0 1px 1px rgba(0,0,0,0.24)","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,1)","width":"100%"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('lingshixinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('lingshixinxi','购买')" @click="goumailingshiCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								购买
							</el-button>
							<el-button class="edit" v-if=" isAuth('lingshixinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('lingshixinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('lingshixinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<goumailingshi-cross-add-or-update v-if="goumailingshiCrossAddOrUpdateFlag" :parent="this" ref="goumailingshiCrossaddOrUpdate"></goumailingshi-cross-add-or-update>
...
import goumailingshiCrossAddOrUpdate from "../goumailingshi/add-or-update";
	export default {
		data() {
			return {
				...
				goumailingshiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			goumailingshiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.goumailingshiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','lingshixinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.goumailingshiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.goumailingshiCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	lingshixinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'lingshixinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
