...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"120px 0 0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 220px)","float":"left","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>包厢名称：</div>
        <el-input v-model="formSearch.baoxiangmingcheng" placeholder="包厢名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>包厢类型：</div>
        <el-select v-model="formSearch.baoxiangleixing" placeholder="请选择包厢类型" :clearable="true">
          <el-option v-for="(item, index) in baoxiangleixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>包厢状态：</div>
        <el-select v-model="formSearch.baoxiangzhuangtai" placeholder="请选择包厢状态" :clearable="true">
          <el-option v-for="(item, index) in baoxiangzhuangtaiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('baoxiangxinxi','新增')" :style='{"cursor":"pointer","border":"1px solid #170000","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/baoxiangxinxiAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"3px solid #fff","width":"45%","padding":"8px","overflow":"hidden","height":"100%"}' class="img-box">
				<img @click.stop="imgPreView(item.baoxiangtupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.baoxiangtupian && item.baoxiangtupian.substr(0,4)=='http'" :src="item.baoxiangtupian" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.baoxiangtupian?item.baoxiangtupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.baoxiangtupian?item.baoxiangtupian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"padding":"0 10px","overflow":"hidden","alignItems":"center","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"auto","padding":"0 10px 0 0","flexDirection":"column","flex":"1","display":"flex"}'>
				<div class="title">{{item.baoxiangmingcheng}}</div>
				<div class="title">{{item.baoxiangleixing}}</div>
				<div class="title">{{item.xiaoshijiage}}</div>
		        ...
		        <div :style='{"padding":"0 10px","borderColor":"red","borderStyle":"solid","borderWidth":"0"}' class="like">
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('baoxiangfenlei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'baoxiangfenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.baoxiangmingcheng != '') searchWhere.baoxiangmingcheng = '%' + this.formSearch.baoxiangmingcheng + '%';
        if (this.formSearch.baoxiangleixing != '') searchWhere.baoxiangleixing = this.formSearch.baoxiangleixing;
        if (this.formSearch.baoxiangzhuangtai != '') searchWhere.baoxiangzhuangtai = this.formSearch.baoxiangzhuangtai;
        if (this.curFenlei != '全部') searchWhere.baoxiangfenlei = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`baoxiangxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/baoxiangxinxiDetail', query: params});
      },
