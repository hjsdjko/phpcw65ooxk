...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"120px 0 0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 220px)","float":"left","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>零食名称：</div>
        <el-input v-model="formSearch.lingshimingcheng" placeholder="零食名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>零食分类：</div>
        <el-select v-model="formSearch.lingshifenlei" placeholder="请选择零食分类" :clearable="true">
          <el-option v-for="(item, index) in lingshifenleiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
	  ...
	  <el-button v-if="btnAuth('lingshixinxi','新增')" :style='{"cursor":"pointer","border":"1px solid #170000","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/lingshixinxiAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"3px solid #fff","width":"45%","padding":"8px","overflow":"hidden","height":"100%"}' class="img-box">
				<img @click.stop="imgPreView(item.tupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian" class="image" />
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","alignItems":"center","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"auto","padding":"0 10px 0 0","flexDirection":"column","flex":"1","display":"flex"}'>
				<div class="title">{{item.lingshimingcheng}}</div>
		      ...
		      <div class="desc">{{item.lingshixiangqing}}</div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/lingshifenlei/lingshifenlei').then(res => {
			if (res.data.code == 0) {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('lingshifenlei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'lingshifenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.lingshimingcheng != '') searchWhere.lingshimingcheng = '%' + this.formSearch.lingshimingcheng + '%';
        if (this.formSearch.lingshifenlei != '') searchWhere.lingshifenlei = this.formSearch.lingshifenlei;
        if (this.formSearch.pinpai != '') searchWhere.pinpai = '%' + this.formSearch.pinpai + '%';
        if (this.curFenlei != '全部') searchWhere.lingshifenlei = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`lingshixinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/lingshixinxiDetail', query: params});
      },
